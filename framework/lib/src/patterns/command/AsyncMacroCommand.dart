...
class AsyncMacroCommand extends Notifier implements IAsyncCommand
{
	...
	List<Function> _subCommands;
	INotification _note;
	int _counter = 0;

	AsyncMacroCommand() {
		_subCommands = new List<Function>();
	}
...
//		print("> Framework -> AsyncMacroCommand -> executeNextCommand: _counter = ${_counter}");
		if (_subCommands.length > _counter) _nextCommand();
		else {
	...
	void _nextCommand() {
		...
		if (isAsync) (commandInstance as IAsyncCommand).setOnComplete( _executeNextCommand );
